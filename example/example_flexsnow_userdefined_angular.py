...
def main():
    ...
    working directories
    """
    work_dir = Path(r"E:\WORK\FsnowV2_2026\Test\user_defined")
    ...
    NPHI = -3
    NUMU = -3
    geometry = np.array([[0,0,0,0],[10,0,0,0]])
    irrDir = 1.0
    ...
    fileGEO = work_dir / f"input_geometry.txt"
    ...
    for wavelength in wavelengths:
        ...
        flag = f"wvl={wavelength:.3f}um.txt"

        instance = MLDISORTv2(irrDir=irrDir, NLYR=nlyr, NPHI=NPHI, NUMU=NUMU, geometry=geometry,
                              loadAng=loadAng, append=append_tf,NSTR=NSTR, NMOM=NMOM, density=density, depth=thickness)
        ...
        instance.fileGEO = fileGEO
        instance.write_angles_file()
        instance.run()
